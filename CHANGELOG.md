...
- **FPF corpus refresh to `34b4d63`** — submodule `data/FPF` bumped one commit upstream (b18acde → 34b4d63, "temporal claim adequacy"). Embedded index regenerated via `task fpf-refresh`: indexed_chunks 4961 → 4972 (+11; 4907 spec + 65 patterns), fpf_commit matches submodule HEAD. Index and submodule pointer move together — the release workflow rebuilds the index from the locked submodule SHA on tag, so a drift between these two files would surface as either stale search results in dev or a mismatch on next release build.
