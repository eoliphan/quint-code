...
func TestSelfHostGoDetection(t *testing.T) {
	// Find haft project root (walk up from test file)
	root := findProjectRoot(t)
	if root == "" {
		t.Skip("could not find haft project root")
	}
	...
	if len(modules) == 0 {
		t.Fatal("expected to detect Go modules in haft")
	}
	...
	t.Logf("Detected %d Go modules in haft", len(modules))
	for _, m := range modules {
...
func findRepoRoot(t *testing.T, repoName string) string {
	t.Helper()
	// Walk up to find haft project root, then check .context/repos/<repoName>
...
func findProjectRoot(t *testing.T) string {
	t.Helper()
	// Walk up from current directory looking for go.mod with haft module
	dir, err := os.Getwd()
	if err != nil {
		return ""
	}
