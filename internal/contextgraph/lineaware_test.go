...
// the symbol level, must NOT bleed governance across each other. A line inside
// one overload's body resolves to its decision only; a line-blind query
// surfaces both but is honestly labeled, never presenting false precision.
func TestFetchCodeContext_LineAwareOverloadSeparation(t *testing.T) {
	...
	if ccA.SymbolGranularity != "line-precise" {
		t.Fatalf("expected line-precise granularity, got %q", ccA.SymbolGranularity)
	}
	...
	if ccBoth.SymbolGranularity != "file+name (overload not disambiguated)" {
	...
	if ccFallback.SymbolGranularity != "file+name (overload not disambiguated)" {
