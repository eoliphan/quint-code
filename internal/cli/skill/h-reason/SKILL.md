...
name: h-reason
description: |
  Umbrella for FPF-style structured reasoning in a haft project. Carries the whole reasoning palette in one place: framing, exploration, comparison, verification, notes, and the slideument wisdom (NQD, Goldilocks, stepping stones, BLP, Scaling-Law Lens). Make sure to use this skill whenever the operator wants structured thinking but the specific workflow isn't pre-named — phrases like "давай подумаем", "помоги разобраться", "let's think this through", "structured approach", "apply FPF here", "FPF reasoning", "haft this", "use FPF in your thinking" — or whenever a request is ambiguous between framing, exploration, and comparison. Also a manual entry point — operators can always type /h-reason or /h-fpf (alias) to enter the umbrella. For sharp specific signals the dedicated skills still fire (h-frame, h-diagnose, h-explore, h-compare, h-verify, h-status, h-note, h-onboard, h-spec-cover); for binding decisions use manual /h-decide; for commissioning use manual /h-commission. Anything except those two is fair game for this umbrella.
when_to_use: |
